...
public class InjectTransform extends Transform {
    ...
    /**
     *  transform像一个链表一样，插入一个Transform将会被先执行，这里当执行到
     *  transformClassesWithXdjaInectClassForDebug 时，即处理Classes时会调到我们的Transform中
     * @param context
     * @param inputs
    ...
    public void transform(
        ...
        Log.info "==============injectClass ${Util.getInjectParam().transformName + ' '}transform enter=============="
//        String flavorAndBuildType = context.name.split("For")[1]
//        Log.info("flavorAndBuildType ${flavorAndBuildType}")
        // 获取在gradle中配置的要插入的代码
        Map<String, Object> modifyMatchMaps = Util.getInjectParam().modifyMatchMaps;
        // targetClass中存放的是要hook的class及类型（正则、通配符等）
        ...
        // 这里的TransformInput即输入，包含jar 和 class目录
        inputs.each { TransformInput input ->
            ...
            Log.info('===============================')
            // 这里的jar，是项目中用到的jar，在日志中你可能会看到很多jar位于
            // .android\build-cache\c3ba1d8edba10e0e0af5c39b05289a60cb868638\output\jars\
            // 这是AndroidStudio的新特性 buildcache，为了加快编译的速度
        ...
        inputs.each { TransformInput input ->
            ...
            input.jarInputs.each { JarInput jarInput ->
                ...
                Log.info("获得输出文件 ：${dest.absolutePath}");
...
//                Log.info("dest dir  ${dest.absolutePath}")
    ...
    private static void saveModifiedJarForCheck(File optJar) {
        File dir = DataHelper.ext.injectClassDir;
